...
3. **Architecture doesn't move the number — proven on tests we took, corroborated from outside.** We ran seven memory-side variants (keyword, hybrid keyword+preference, global-context, chain-of-thought, distillation) through the official scorer at a fixed gpt-4o-mini reader; the best *tied* the simple baseline (63.2% vs 63.0%) and none beat it. **[ours, officially scored]** Independent self-reported systems land in the same range — Zep at 63.8% on gpt-4o-mini, beside our 63.0% — corroboration that we are not an outlier. **[vendor, self-reported]** Reader swaps, by contrast, move scores several points: a published example moved 63.8% → 71.2% by changing only gpt-4o-mini → gpt-4o. **[vendor, self-reported]**
...
- **The benchmarks conflate the two.** End-to-end QA accuracy is reader-capped ([thesis, Moves 1–3](./reader-is-the-bottleneck.md)), yet it is reported as a *memory* score. Vendors optimize and market against a number their component cannot move.
...
All figures and their labels: [docs/benchmarks.md](../../docs/benchmarks.md). Component-level argument and the reader-swap corroboration: [The Reader Is the Bottleneck](./reader-is-the-bottleneck.md). External figures cited remain **[vendor, self-reported]** per their sources.
