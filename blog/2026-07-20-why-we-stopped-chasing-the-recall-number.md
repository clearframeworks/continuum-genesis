...
That's not a memory problem. That's a ceiling that belongs to the model doing the answering — and no amount of memory engineering climbs past it. We wrote the full argument up as [The Reader Is the Bottleneck](./thesis-llm-bottleneck.md), with every number labeled by how it was produced in the [benchmark results](../docs/benchmarks.md).
...
The industry mostly sells the other story — memory as intelligence, an AI that "grows with you." We think that's a mirage, and we wrote down why, including exactly what result would prove us wrong: [The Persistent Mirage](./the-persistent-mirage.md).
